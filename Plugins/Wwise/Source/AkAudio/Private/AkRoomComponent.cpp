...
void UAkRoomComponent::SetAttenuationScalingFactor(float InAttenuationScalingFactor)
{
	...
	{
		AttenuationScalingFactor = 1.f;
		UE_LOG(LogAkAudio, Warning, TEXT("UAkRoomComponent::SetAttenuationScalingFactor: Atteanution scaling factor for Room %s is zero or a negative number. It has been set to 1."), *GetRoomName());
	}
...
void UAkRoomComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction * ThisTickFunction)
{
	...
	if (bAttenuationScalingFactorNeedsUpdate &&
		bIsRegisteredWithWwise &&
		AkAudioEvent != nullptr &&
