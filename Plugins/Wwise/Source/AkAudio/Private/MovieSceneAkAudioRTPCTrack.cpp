...
FText UMovieSceneAkAudioRTPCTrack::GetDisplayName() const
{
	...
	if (AllSections.Num() > 0)
	{
		...
		DisplayName += TEXT(" - ") + (RTPCSection->GetRTPC() ? RTPCSection->GetRTPC()->GetName() : TEXT("Unassigned"));
	}
...
FName UMovieSceneAkAudioRTPCTrack::GetTrackName() const
{
	...
	return (Section != nullptr && Section->GetRTPC()) ? FName(*Section->GetRTPC()->GetName()) : FName(NAME_None);
}
