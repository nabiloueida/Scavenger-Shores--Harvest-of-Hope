...
void UAkGeometryComponent::UpdateGeometry()
{
	if (Parent)
	{
		...
		SendGeometryInstanceToWwise(Parent->GetComponentRotation(), Parent->GetComponentLocation(), Parent->GetComponentTransform().GetScale3D(), roomID, !bWasAddedByRoom, bSolid, bBypassPortalSubtraction);
	}
}
