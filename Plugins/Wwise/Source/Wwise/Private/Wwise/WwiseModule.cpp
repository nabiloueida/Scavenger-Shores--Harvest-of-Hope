...
#include "Wwise/WwiseAssetLibraryRuntimeModuleImpl.h"
#if WITH_EDITOR
...
public:
	virtual void StartupModule() override
	{
		...
		GConfig->GetBool(TEXT("/Script/AkAudio.AkSettings"), TEXT("bPackageAsBulkData"), bPackageAsBulkData, GGameIni);
		if (bPackageAsBulkData || WITH_EDITOR)
		{
			{
				SCOPED_WWISE_EVENT(TEXT("StartupModule: WwiseAssetLibrary Runtime"));
				UE_LOG(LogWwise, Log, TEXT("WwiseModule: Loading WwiseAssetLibrary"));
				IWwiseAssetLibraryRuntimeModule::GetModule();
			}
#if WITH_EDITOR
			{
				SCOPED_WWISE_EVENT(TEXT("StartupModule: WwiseAssetLibrary Editor"));
				IWwiseAssetLibraryEditorModule::GetModule();
			}
#endif
		}
		else
		{
			UE_LOG(LogWwise, Verbose, TEXT("WwiseModule: WwiseAssetLibrary is disabled without Bulk Data Packaging. Ignoring."));
		}
